...
class CalculateRankingTest {
    ...
    @Test
    fun `with many game days with many match each`() {
        // given
        val amateurSoccerGroupId = UUID.randomUUID()
        val player1 = UUID.randomUUID()
        val player2 = UUID.randomUUID()
        ...
        val playerStatisticsRepository = TestDoublePlayerStatisticsRepository()
        val date1 = LocalDate.of(2021, 5, 1)
        playerStatisticsRepository.persist(amateurSoccerGroupId, PlayerStatistic(player1, date1, 3, 2, 1, 0, 20, 12))
        playerStatisticsRepository.persist(amateurSoccerGroupId, PlayerStatistic(player2, date1, 3, 1, 1, 1, 17, 15))
        playerStatisticsRepository.persist(amateurSoccerGroupId, PlayerStatistic(player3, date1, 3, 1, 1, 1, 15, 17))
        playerStatisticsRepository.persist(amateurSoccerGroupId, PlayerStatistic(player5, date1, 1, 0, 1, 0, 5, 5))
        playerStatisticsRepository.persist(amateurSoccerGroupId, PlayerStatistic(player4, date1, 2, 0, 0, 2, 7, 15))
